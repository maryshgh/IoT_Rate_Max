...
c = 4.88;
c11 = c*exp(-b*(pi/2-c));
c22 = .5*(b^2*(1-delta)+r*(c11-1));

ub = max(x)*ones(1,nVar); 
lb = min(x)*ones(1,nVar);
...
for t = 1 : maxIter
    ...
    for k = 1 : noP
        ...
        Swarm.Particles(k).O = fobj(currentX,nVar,r,x,h,delta,P,b,c);
...
cost_opt = -Swarm.GBEST.O
deploy_opt = Swarm.GBEST.X
semilogy(cgCurve);
